...
func (m *manager) updateContainerProbeStatus(pod *v1.Pod, containerStatus *v1.ContainerStatus) {
	...
	if exist {
		containerStatus.StartupProbe = w.currentSpec
	} else {
		containerStatus.StartupProbe = nil
	}
	// status of readiness probe
	w, exist = m.getWorker(pod.UID, containerStatus.Name, readiness)
	if exist {
		containerStatus.ReadinessProbe = w.currentSpec
	} else {
		containerStatus.ReadinessProbe = nil
	}
	// status of liveness probe
	w, exist = m.getWorker(pod.UID, containerStatus.Name, liveness)
	if exist {
		containerStatus.LivenessProbe = w.currentSpec
	} else {
		containerStatus.LivenessProbe = nil
	}
}
